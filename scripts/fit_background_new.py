...
from cmdstanpy import cmdstan_path, CmdStanModel, from_csv
...
stan_model_db = "/u/gbmtrans/scripts/bkg_pipe/stan_models/"
stan_model_dir = os.path.join(
    ...
    f"{'gc' if config['setup']['use_gc'] else 'no_gc'}",
...
# Sample
if os.path.exists(os.path.join(output_dir, "stan_chains/", "sample_success")):
    with open(os.path.join(output_dir, "stan_cains/", "sample_success"), "r") as f:
        fit_success_date = f.read()
        print(
            f"Found existing fit result from {fit_success_date}\n Using the existing Stan csv files for exporting"
        )
    import re

    csvs = [
        os.path.join(output_dir, "stan_chains", i)
        for i in os.listdir(os.path.join(output_dir, "stan_chains"))
        if re.search("background_model-[0-9]{12}-[0-9]*.csv", i)
    ]
    stan_fit = from_csv(csvs, method="sample")
    ...
    time_fit = datetime.now() - start_fit
    with open(os.path.join(output_dir, "stan_chains/", "sample_success"), "w+") as f:
        f.write((datetime.now() - start_fit))


start_export = datetime.now()
fn = "fit_result_{}_{}_e{}.hdf5".format(
...
if not os.path.exists(os.path.join(output_dir, fn)):
    ...
    stan_model_file_export = os.path.join(
        stan_model_dir, "background_model_export.stan"
    )
